...
@Component({
    selector: 'app-paidloan',
    templateUrl: './paidloan.component.html',
    styleUrls: ['./paidloan.component.css']
})
export class PaidloanComponent implements OnInit {
    ...
    getLoans() {
        this.loading = true
        this.operationsService.getClosedLoanPortfolio(this.currentUser.token, this.filter).subscribe(data => {
            this.loading = false;
