...
@Component({
  selector: 'app-directdebitinitiate',
  templateUrl: './directdebitinitiate.component.html',
  styleUrls: ['./directdebitinitiate.component.css']
})
export class DirectdebitinitiateComponent implements OnInit {
  ...
  doSendDebitInstruction(value: any): void {
    ...
    this.loading = true;
    this.operationsService.doSendDebitInstruction(this.currentUser.token, value, this.repayment,this.directdebitrequest)
      .subscribe(status => {
        this.loading = false;
        if (status.status == true) {
