...
@Component({
  selector: 'app-clearingfines',
  templateUrl: './clearingfines.component.html',
  styleUrls: ['./clearingfines.component.css']
})
export class ClearingfinesComponent implements OnInit {
  ...
  doSendReverseTransaction(value: any): void {
    ...
    this.loading = true;
    this.operationsService.doClearFines(this.currentUser.token, value, this.repayment,this.directdebitrequest)
      .subscribe(status => {
        this.loading = false;
        if (status.status == true) {
