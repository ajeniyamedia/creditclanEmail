...
@Component({
  selector: 'app-makeloanpayment',
  templateUrl: './makeloanpayment.component.html',
  styleUrls: ['./makeloanpayment.component.css']
})
export class MakeloanpaymentComponent implements OnInit {
  directdebitrequest: any;
  debit_all_card: any;
  confirmCardAD = false;
  loan: any;
  cards: any;
  ...
  doSendReverseTransaction(value: any): void {
    this.makingFinalPayment = true;
    this.paymentHasBeenProcessed = false;
    this.payment_status = false;
    this.loading = true;
    this.operationsService.doSendReverseTransaction(this.currentUser.token, value, this.repayment,this.directdebitrequest)
      .subscribe(status => {
        this.loading = false;
        if (status.status == true) {
  ...
  ngOnInit() {
    this.getBanks();
    this.loansService.getLoanCardsAndAccounts(this.currentUser.token, this.repayment.REQUEST_ID)
      .subscribe(loan => {
        this.accounts = loan.accounts;
        this.directdebitrequest = loan.directdebitrequest;
      });
    this.makingFinalPayment = false;
    this.paymentHasBeenProcessed = false;
    this.otpError = false;
      ...
      if (this.sub != '0') {
        ...
        switch (this.schedule_type) {
          case '1':
            ...
            if (this.record_type == '2') {
              this.processDisbursement(this.disburse);
              this.payment_type = 'Withdrawals';
            }
            if (this.record_type == '4') {
              this.processDisbursement(this.disburse);
              this.payment_type = 'Wallet Withdrawal';
            }
            if (this.record_type == '5') {
              this.processDisbursement(this.disburse);
              this.payment_type = 'Customer Wallet Funding';
            }
            if (this.record_type == '7') {
              this.processDisbursement(this.disburse);
              this.payment_type = 'Customer Payment Rollback';
            }
            break;
          case '2':
            if (this.record_type == '3') {
            ...
            if (this.record_type == '6') {
              this.processDisbursement(this.disburse);
              this.payment_type = 'Customer Repayment';
            }
            break;
        }
      }
  ...
  useAsDirectDebitAccount(ACCOUNT_CARD_ID, DIRECT_DEBIT_REQUEST) {
    this.directdebitrequest = DIRECT_DEBIT_REQUEST;
  }
}
