...
export class OfferLetterComponent implements OnInit {
  ...
  formCheckboxCustomerDetails: FormGroup;
  ...
  mailSubjectSection: any;
  vendorSetting: any;
  vendorSettingData: any;
  vendorCustomerDetailsSettingData: any;
  logoUrl: string;
  CustomerSetting: any;
  ...
  constructor(private fb: FormBuilder,
    ...
    private services: OfferletterService) {
  ...
  updateSubjectContentForm = new FormGroup({
  ...
  offerLetterFieldSetting = new FormGroup({
    contract_id: new  FormControl(''),
    loan_amount: new  FormControl(''),
    duration: new  FormControl(''),
    fees: new  FormControl(''),
    net_disbursement: new  FormControl(''),
    monthly_installment: new  FormControl(),
    total_repayment: new  FormControl(''),
    client_no: new  FormControl('')
  });


  offerLetterCustomerDetailsFieldSetting = new FormGroup({
  ...
  updateCustomerDetailsSeting(){
    const selectedCustomerDetails = this.formCheckboxCustomerDetails.value.fieldsCustomerDetails;
    ...
    const updateCustomerDetails = {
      ...
      'token': this.token,
      'address': selectedCustomerDetails[0].length == 0 ? this.vendorCustomerDetailsSettingData.address : selectedCustomerDetails[0],
      'cc_number': selectedCustomerDetails[1].length == 0 ? this.vendorCustomerDetailsSettingData.cc_number : selectedCustomerDetails[1],
      'legal_name': selectedCustomerDetails[2].length == 0 ? this.vendorCustomerDetailsSettingData.legal_name : selectedCustomerDetails[2],
      'date': selectedCustomerDetails[3].length == 0 ? this.vendorCustomerDetailsSettingData.date : selectedCustomerDetails[3]
    };

    console.log(updateCustomerDetails);

    this.services.addCustomerDetailsSetting(updateCustomerDetails)
      .subscribe(onCustomerDetailsUpdated => {
        this.loading = false;
        this.fetchOfferLetterCustomerDetailsFieldSetting ();
  ...
  onUpdateFooterContent(file){
   ...
   ////////////////////////////////////////////////////////////////////////
  //// Get offer letter customer details from the database

  fetchOfferLetterCustomerDetailsFieldSetting(){
    ...
    this.services.getCustomerDetails(uploadData)
      .subscribe(customerDetails => {
        this.vendorCustomerDetailsSettingData  = customerDetails;

        this.fieldsCustomerDetails = [
          { name: 'Legal Name', value: customerDetails.legal_name == 1 ? true : false },
          { name: 'CC Number', value: customerDetails.cc_number == 1 ? true : false },
          { name: 'Address', value: customerDetails.address == 1 ? true : false },
          { name: 'Date' , value: customerDetails.date == 1 ? true : false  }
        ];

        // Create a new array with a form control for each order
          const controls = this.fieldsCustomerDetails.map(c => new FormControl(''));
          this.formCheckboxCustomerDetails = this.fb.group({
            fieldsCustomerDetails: new FormArray(controls)
          }); 
    });
  }
  ...
  ngOnInit() {
    ...
    this.services.getOfferLetterContent(getBodyContent)
      .subscribe(bodyContent => {
        this.mailBodyContent = bodyContent[0].content;
    }); 

    ////////////////////////////////////////////////////////////////////
    // Get mail subject content

    const getSubjectContent = {
      'vendor_id': this.vendor_id,
      'email_type': this.email_type,
      'section_type': this.mailSubjectSection,
      'token': this.token
    };


    this.services.getOfferLetterContent(getSubjectContent)
      .subscribe(bodyContent => {
        this.mailSubjectContent = bodyContent[0].content;
    }); 
    ...
    this.services.getOfferLetterContent(getLogoContent)
    ...
    this.services.getOfferLetterContent(getFooterContent)
      .subscribe(footerContent => {
        this.footerBodyContent = footerContent[0].content;
    }); 

    ////////////////////////////////////////////////////////////////////
     // Get customer details setting
    this.fetchOfferLetterCustomerDetailsFieldSetting();
  ...
  onSubmit() {
    ...
    this.services.uploadOfferLetterLogo(uploadData)
